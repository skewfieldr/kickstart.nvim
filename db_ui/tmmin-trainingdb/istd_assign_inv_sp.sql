...
BEGIN
	...
	RAISE NOTICE 'inv id %s', p_inventory_id;
	...
	RAISE NOTICE 'Success insert inventory assignment log';
	-- COMMIT;
	...
	THEN
		...
		RAISE NOTICE 'Success update inventories table';
	ELSE 
		RAISE NOTICE 'Inventory is already assigned to an employee';
		ROLLBACK;
		RETURN;
	END IF;
